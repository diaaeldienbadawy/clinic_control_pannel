...
export const SubService = () => {
    const control = useVM<SubServiceVM>(new SubServiceVM())
      ...
      <Container className='py-3'>
        <Row className=' justify-content-center'>
          <Col md={6} lg={4} xl={3} className='px-0'>
            <h4 className='text-center'>اضافة خدمة جديد</h4>
            <div className='py-2 d-flex justify-content-between'>
              <input 
                onKeyDown={a=>Essentials.EnterKeyPressHandling(a,()=>addRef.current?.click())} 
                type="text" className='input-field w-100' value={control.NewItem?control.NewItem.service:''} 
                onChange={a=>control.NewItem = control.NewItem?{ ...control.NewItem,service:a.target.value}:{service:a.target.value , duration:'00:00'}} 
              />
        ...
        control.ShowWithMessege(
          control.Data.map((item,index)=>{
            const Item = ()=>{
              ...
              const [value, setValue] = useState(item?.service)
              const [duration, setDuration] = useState(item?.service)
              const ref = useRef<HTMLSpanElement>(null)
              const [deleting, setDeleting] = useState(false)
            
              return(
                <Col key={index} md={6} lg={4} className='px-md-2 py-3 px-0 h-100'>
                  {
                    deleting?
                    (<div className='p-2 rounded-2 border d-flex justify-content-between'>
                      <div>هل تريد حذف الخدمة .. {value} ؟</div>
                    ...
                    <div className='py-1 px-md-2 rounded-2 border row justify-content-between  h-100'>
                      <div className='col-sm-auto col-12  px-2 d-flex'>
                        {
                          edit?(
                      ...
                      <div className='my-auto col-sm-auto col-12 g-5  d-flex justify-content-end px-md-3 p-1 '>
                        {
                          edit?(
                            <span ref={ref} className='material-symbols-outlined c-pointer btn-clr-save ' onClick={()=>{ control.edit({id:id , service:value??'' , duration:duration??'00:00'},()=>setEdit(false))}}>save</span>
                          ):(
...
export class SubServiceVM extends ListVM<subService>{
    constructor(){
        super(SubServiceClient,false)
    }
}